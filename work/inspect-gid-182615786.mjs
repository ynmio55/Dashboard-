...
async function run() {
  ...
  console.log(`Total active rows: ${records.length}`);
  records.forEach((row, idx) => {
    console.log(`Row ${idx}: Col0="${row[0] || ''}" | Col15_21="${row.slice(15, 21).join(', ')}" | Col36_42="${row.slice(36, 42).join(', ')}" | Col43_47="${row.slice(43, 48).join(', ')}"`);
  });
}
